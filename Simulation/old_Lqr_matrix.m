...
%列出替代方程
Nm(t) = M*diff(diff(x+(L+Lm)*sin(theta)-l*sin(phi),t) , t);%(1.7)
Pm(t)= M*g + M*diff(diff((L+Lm)*cos(theta)+l*cos(phi),t), t);%(1.8)
N(t) = Nm(t) + mp*diff(diff(x + L*sin(theta), t), t);%(1.4)
P(t) = Pm(t) + mp*g + mp*diff(diff(L*cos(theta), t), t);%(1.5)
%简化，用字母替代
Nm(t) = (subs(Nm(t), ...
...
eqns = [x_dot_dot == (T-N(t)*R)/(Iw/R + mw*R);
    Ip*theta_dot_dot == (P(t)*L+Pm(t)*Lm) * sin(theta)-(N(t)*L+ Nm(t)*Lm) * cos(theta)- T(t) + Tp(t);%(1.6)
    Im*phi_dot_dot == Tp(t)+Nm*l*cos(phi)+Pm*l*sin(phi)];%(1.9)
